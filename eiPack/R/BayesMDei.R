BayesMDei <- function(formula, data, total, lambda1 = 4, lambda2 = 2,
                      tune.alpha = NULL, tune.beta = NULL,
                      start.alphas = NULL, start.betas = NULL,
                      sample = 1000, noise_scale = 10, thin = 1, burnin = 1000, verbose = 0,
                      ret.beta ='r', ret.mcmc = TRUE, privacy = FALSE, pois_parameter = pois_parameter, ...){
...
# }
  ...
  if(identical(round(apply(start.betas, c(1,3), sum),10),
               matrix(1,NG, Precincts))!=TRUE){
    stop('inadmissable starting values for beta')}
...
ret.beta')}
  ...
  output <- .Call("rbycei_fcn1",
                  ...
                  as.numeric(XX_noisy),
                  as.numeric(TT_noisy),
                  as.numeric(noise_scale),
                  as.numeric(privacy),
                  as.numeric(pois_parameter)
                  )

  if(ret.beta==0){names(output) <- c("Alpha", "Beta", "alpha.acc",
                                  "beta.acc", "cell.count")}
  else{names(output) <- c("Alpha", "alpha.acc", "beta.acc","cell.count")}
  ...
  dimnames(output$cell.count) <- list(group.names, party.names, 1:sample)
  if(ret.beta==0){
