...
double xx_total_ll (double xx_total, double xx_total_noisy, double noise_scale, double pois_parameter) {
  double ll;
  ll = log(lgammafn(xx_total + 1)) + xx_total*log(pois_parameter) - (1 / (2*noise_scale*noise_scale))*(xx_total - xx_total_noisy)*(xx_total - xx_total_noisy);
  return(ll);
}
